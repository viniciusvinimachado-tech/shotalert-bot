...
API_KEY = os.environ.get("API_KEY", "0b25f4c3636f1848cd546828c84a96d9")
...
# ─── LIGAS COM TEMPORADA AUTO ─────────────────────────────────────────────────
def get_season(league_id):
    hoje = date.today()
    ano = hoje.year
    mes = hoje.month

    # Ligas europeias: temporada vira em julho. Ex: maio/2026 = season 2025
    ligas_europa = ["2", "3", "848", "39", "140", "135", "78", "61", "88", "94"]
    if str(league_id) in ligas_europa:
        return str(ano - 1) if mes < 7 else str(ano)

    # Ligas anuais: Brasil, Libertadores, MLS, Argentina
    ligas_anuais = ["71", "73", "13", "11", "253", "262", "128"]
...
def buscar_jogos_data(data_str, league_id, season):
    ...
    try:
        ...
        if "errors" in data and data["errors"]:
            logging.error(f"API ERR L{league_id}: {data['errors']}")
        if "message" in data:
            logging.error(f"API MSG L{league_id}: {data['message']}")
        if "results" in data:
            logging.info(f"API Results: {data['results']}")

        result = data.get("response", [])
        logging.info(f"API {league_id} {season} {data_str}: {len(result)} jogos")
    except Exception as e:
        logging.error(f"Exception L{league_id}: {e}")
        result = []
...
def buscar_stats(team_id, league_id, season):
    ...
    try:
        ...
        logging.error(f"Erro buscar_stats: {e}")
        data = {}
...
def buscar_artilheiros(team_id, league_id, season):
    ...
    try:
        ...
        logging.error(f"Erro buscar_artilheiros: {e}")
        players = []
    finalizadores = []
    for p in players:
...
def buscar_resultado_jogo(fixture_id):
    try:
        ...
        logging.error(f"Erro buscar_resultado_jogo: {e}")
        return None, None
...
def relatorio_diario(context):
    ...
    msg += f"✅ {len(acertos)} | ❌ {len(alertas_hoje)-len(acertos)} | 🎯 *{pct}%*\n\n"
...
def start(update, context):
    update.message.reply_text(
        "🤖 *Finalizações Bot v6.2*\n\n"
        "🌍 17 ligas • 🎯 Filtro 75%+ • 📊 Histórico\n"
        "📅 Temporada automática • 🕐 Timezone UTC\n\n"
        "/jogos — Jogos de hoje\n"
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.2 online! 🌍 17 ligas | 🎯 Filtro 75%+ | 📅 Season auto | 🕐 UTC")
    ...
    for nome, info in LIGAS.items():
        try:
            resp = requests.get(
                f"{BASE_URL}/fixtures",
                headers=HEADERS,
                params={"date": hoje, "league": info["id"], "season": info["season"], "timezone": "UTC"},
                timeout=10
            )
            data = resp.json()
            count = len(data.get("response", []))
        except Exception as e:
            logging.error(f"Exception {nome}: {e}")
            count = 0
        if count > 0:
...
if __name__ == "__main__":
    ...
    logging.info(f"Bot v6.2 rodando — API-Football! Porta: {PORT}")
