...
def buscar_jogos_data(data_str, league_id, season):
    ...
    try:
        ...
        logging.error(f"Erro buscar_jogos_data: {e}")
        result = []
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.0 online! 🌍 17 ligas | 🎯 Filtro 75%+")
...
def main():
    ...
    # Sobe Flask em thread separada pra rota /health
    Thread(target=run_flask, daemon=True).start()
    logging.info("Flask iniciado para /health")
    ...
    # Webhook pro Railway / Polling local
    RAILWAY_URL = os.environ.get("RAILWAY_STATIC_URL")

    if RAILWAY_URL:
        updater.bot.delete_webhook(drop_pending_updates=True)
        logging.info(f"Iniciando webhook em {RAILWAY_URL}")
        # Porta do webhook é a mesma do Flask, mas o python-telegram-bot gerencia internamente
        # O Flask só serve a /health. O webhook usa o servidor interno do Updater.
        ...
        logging.info("Rodando local com polling")
        updater.bot.delete_webhook(drop_pending_updates=True)
        updater.start_polling(drop_pending_updates=True)

    logging.info("Bot v6.0 rodando — API-Football!")
    updater.idle()
