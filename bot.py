...
def buscar_artilheiros(team_id, league_id="71"):
    key = f"artilheiros_{team_id}_{league_id}"
    cached = cache_get(key)
    if cached:
        return cached
...
def calcular_importancia(fixture):
    ...
    if "Quarter" in rodada or "Quartas" in rodada:
        return 8
    if liga_id == "73":  # Copa do Brasil
        return 8
    if liga_id == "13":  # Libertadores
        return 9
    return 7
...
def analisar_jogos(fixtures, dias_restantes=0, limite=5, min_confianca=65):
    alertas = []
    for f in fixtures[:limite]:
        ...
        if total >= 18:
            fin_casa = buscar_artilheiros(id_casa, league_id)
            fin_fora = buscar_artilheiros(id_fora, league_id)
...
def main():
    ...
    # Alerta automático todo dia às 08h BRT (11h UTC)
