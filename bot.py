...
def buscar_stats(team_id, league_id, season):
    ...
    ofensividade = min(round((media / 20) * 100), 100) # CORRIGIDO
...
if __name__ == "__main__":
    ...
    # IMPORTANTE: Inicia o updater pra job_queue funcionar
    updater.start_webhook(listen="0.0.0.0", port=PORT, url_path=TOKEN, webhook_url=f"{os.environ.get('RAILWAY_STATIC_URL')}/{TOKEN}")
    updater.idle() # Mantém o updater vivo pra processar jobs
