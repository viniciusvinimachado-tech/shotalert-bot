...
LIGAS = {
    ...
    "Copa do Brasil": {"id": "73", "season": "2024"},
    "Libertadores": {"id": "13", "season": "2024"},
}
...
def analisar_jogos(fixtures, league_id, season, dias_restantes=0, limite=5, min_confianca=50):
    alertas = []
...
def alerta_automatico(context):
    ...
    bot = Bot(token=TOKEN)
    hoje = date.today()
    bot.send_message(chat_id=CHAT_ID, text="🌅 *Bom dia! Analisando jogos...*", parse_mode="Markdown")
    encontrou = False
    for dias in range(4):
    ...
    if not encontrou:
        bot.send_message(chat_id=CHAT_ID, text="📭 Nenhuma oportunidade hoje.", parse_mode="Markdown")
...
def diagnostico(update, context):
    update.message.reply_text("🔬 Testando API nas duas temporadas...")
    hoje = date.today().strftime("%Y-%m-%d")
    resultado = ""
    for season in ["2024", "2025"]:
        for nome, info in LIGAS.items():
            url = "https://api-football-v1.p.rapidapi.com/v3/fixtures"
            params = {"date": hoje, "league": info["id"], "season": season}
            try:
                resp = requests.get(url, headers=HEADERS, params=params, timeout=10)
                ...
                count = -1
            resultado += f"{nome} {season}: {count} jogos\n"
