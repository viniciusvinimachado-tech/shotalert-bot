...
# MAPEAMENTO FD -> SPORTMONKS - NOMES EXATOS DA API SM
MAPEAMENTO_SM = {
    # Premier League
    ...
    "AFC Bournemouth": 35,
    "Luton Town FC": 58,
    "Sheffield United FC": 54,
    "Burnley FC": 50,

    # La Liga
    "Rayo Vallecano de Madrid": 89,
    "Girona FC": 164,
    "RC Celta de Vigo": 85,
    "Levante UD": 93,
    "Real Betis Balompié": 83,
    "Elche CF": 95,
    "CA Osasuna": 88,
    "Club Atlético de Madrid": 82,
    ...
    "Torino FC": 115,
    "Udinese Calcio": 103,
    "US Sassuolo Calcio": 116,
    "US Lecce": 120,
    "Genoa CFC": 107,
    "Empoli FC": 121,
    "Frosinone Calcio": 124,
    "Hellas Verona FC": 118,
    "US Salernitana 1919": 123,
    "Cagliari Calcio": 105,

    # Bundesliga
    "FC Bayern München": 157,
    "Bayer 04 Leverkusen": 162,
    "Borussia Mönchengladbach": 164,
    "RB Leipzig": 174,
    "Borussia Dortmund": 165,
    "Eintracht Frankfurt": 167,
    "VfL Wolfsburg": 158,
    "TSG 1899 Hoffenheim": 168,
    "VfB Stuttgart": 159,
    ...
    "1. FSV Mainz 05": 160,
    "FC Augsburg": 172,
    "SV Werder Bremen": 166,
    "VfL Bochum 1848": 171,
    "1. FC Heidenheim 1846": 175,
    "SV Darmstadt 98": 173,
    ...
    "RC Lens": 74,
    "LOSC Lille": 75,
    "Stade Brestois 29": 77,
    "RC Strasbourg Alsace": 76,
    "Racing Club de Lens": 74,
    "Montpellier Hérault SC": 72,
    "Toulouse FC": 73,
    "FC Nantes": 70,
    "Stade de Reims": 71,
    "Clermont Foot 63": 69,
    ...
    "CR Flamengo": 204,
    "SE Palmeiras": 202,
    "SC Corinthians Paulista": 205,
    "São Paulo FC": 203,
    "Grêmio FBPA": 207,
    "SC Internacional": 208,
    "Cruzeiro EC": 206,
    "Fluminense FC": 201,
    "Botafogo FR": 209,
    "CA Paranaense": 210,
    "EC Bahia": 211,
    "Fortaleza EC": 212,
    "Red Bull Bragantino": 213,
    "Cuiabá EC": 214,
    "CR Vasco da Gama": 215,
...
def buscar_media_stats_sm(nome_time):
    ...
    data = sm_request(f"teams/{team_id}?include=latest.fixtures.statistics.details;latest.fixtures.participants;latest.fixtures.scores")
    if not data or not data.get("data"):
        logging.warning(f"SM: Sem dados {nome_time}")
        return {"media_shots": 0, "media_sot": 0, "media_corners": 0, "media_goals": 0, "jogos": 0}

    fixtures = data["data"].get("latest", [])[:5]
    total_shots = total_sot = total_corners = total_goals = 0
    jogos_validos = 0

    for fix in fixtures:
        if fix.get("state", {}).get("state")!= "FT": continue
...
def formatar_alerta_desregulado(match, nome_liga):
    ...
    # Se não tem stats, não manda alerta
    if media_shots == 0 and media_corners == 0:
        return None
    ...
    if media_goals >= 2.6:
        alertas.append(f"⚽ *Over Gols*\n Projeção: {media_goals} | Buscar: Over 2.5")
...
def start(update, context):
    update.message.reply_text(
        "🤖 *Shot Alert Bot v6.8.2 - Sportmonks*\n\n"
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.8.2 online! Sportmonks mapeado")
...
@app.route('/')
def index():
    return 'Bot v6.8.2 Sportmonks Online'
