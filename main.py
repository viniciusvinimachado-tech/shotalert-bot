...
def buscar_odds_theoddsapi(time_casa, time_fora, sport_key):
    ...
    try:
        ...
        if resp.status_code == 429:
            ...
            logging.error("TheOddsAPI: 429 - Acabou os requests do mês")
            return "SEM_REQUEST"
        ...
        def normalizar(nome):
            nome = nome.lower()
            for cidade in ["los angeles", "golden state", "new york", "san antonio", "oklahoma city",
                          "new orleans", "portland", "indiana", "las vegas"]:
                nome = nome.replace(cidade, "")
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v2.8.8 online!\nTheOddsAPI: {'✅' if ODDS_API_KEY else '❌'}")
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if restantes!= 'N/A':
            cache_set("requests_restantes_oddsapi", int(restantes))
...
@app.route('/')
def index():
    return 'Basket Bot v2.8.8 Online'
