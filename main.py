...
def buscar_top_jogadores_espn(team_id, liga_slug):
    ...
    jogadores = []
    for ath in data["team"]["athletes"][:12]:
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Basket Bot v2.4 online! ESPN + Props OK")

async def hoje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Buscando jogos de hoje na ESPN...")
    ...
    try:
        ...
        if not encontrou:
            await update.message.reply_text("📭 Nenhum jogo hoje na NBA/WNBA/NCAAB.")
        else:
            for msg in msgs:
        ...
        await update.message.reply_text(f"❌ Erro ao buscar jogos: {str(e)[:100]}")

async def amanha(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📅 Analisando jogos de amanhã... ~15s")
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Erro na análise: {str(e)[:100]}")

async def antecipados(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⏰ Buscando odds desreguladas nos próximos 3 dias... ~30s")
    ...
    try:
        ...
        if not encontrou:
            ...
            await update.message.reply_text(f"✅ {total_alertas} oportunidades encontradas.\n\n💡 Casas abrem linha 2-3 dias antes. Pega agora antes de ajustarem!")

    except Exception as e:
        logging.error(f"Erro /antecipados: {e}")
        await update.message.reply_text(f"❌ Erro na busca: {str(e)[:100]}")

async def alerta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔥 Analisando jogos de hoje... ~20s")
    ...
    try:
        ...
        if not quente:
            ...
            await update.message.reply_text(f"✅ {total_enviado} alertas enviados.")
...
@app.route('/')
def index():
    return 'Basket Bot v2.4 ESPN Online'
