...
def cache_get(key):
    item = _cache.get(key)
    if item and time.time() - item["time"] < 86400: # 24h
        return item["data"]
    return None
...
# ─── SPORTMONKS - ENDPOINT GRÁTIS CORRETO ─────────────────────────────────────
...
def buscar_media_stats_sm(nome_time, liga_nome):
    ...
    # ENDPOINT QUE FUNCIONA NO GRÁTIS: /teams/{id}/statistics?season_id=X
    params = {"season_id": season_id}
    data = sm_request(f"teams/{team_id}/statistics", params)
...
def start(update, context):
    update.message.reply_text(
        "🤖 *Shot Alert Bot v6.8.6 - Sportmonks*\n\n"
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.8.6 online! Sportmonks grátis OK")
...
@app.route('/')
def index():
    return 'Bot v6.8.6 Sportmonks Online'
