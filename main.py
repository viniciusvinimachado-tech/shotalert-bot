...
def buscar_odds_theoddsapi(time_casa, time_fora, sport_key):
    ...
    try:
        ...
        def normalizar(nome):
            nome = nome.lower()
            for termo in ["fc", "cf", "ec", "clube", "atlético", "atletico", "real", "ac", "sc"]:
        ...
        for game in dados:
            ...
            # Match por substring nos dois sentidos
            if (casa_norm[:5] in home or home[:5] in casa_norm) and \
               (fora_norm[:5] in away or away[:5] in fora_norm):
                odds_casas = []
...
def formatar_alerta(event, nome_liga, esporte, antecipado=False):
    try:
        ...
        time.sleep(1.5) # Delay maior pra não gastar request
        ...
        if odds_reais:
            for odd in odds_reais:
                ...
                value_min = 0.3 if esporte == "futebol" else 2
                if diferenca >= value_min:
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if total_jogos == 0:
            texto += f"\n😴 Nenhum jogo hoje nas ligas cadastradas."
    ...
    try:
        ...
        for dias in range(1, 4):
            ...
            for nome_liga, info in config["ligas"].items():
                ...
                for g in games[:2]: # 2 jogos por liga
                    try:
...
@app.route('/')
def index():
    return 'Value Bot v2.9.1 Online'
