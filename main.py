...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v2.8.4 online!\nTheOddsAPI: {'✅' if ODDS_API_KEY else '❌'}")
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("🔍 Testando TheOddsAPI...")
    ...
    try:
        ...
        if resp.status_code!= 200:
            await msg.edit_text(f"❌ API Erro {resp.status_code}: {resp.text[:200]}")
            return
        ...
        if len(dados) == 0:
            await msg.edit_text(
                f"✅ API funcionando\n"
                f"😴 0 jogos com odds na NBA\n"
                f"Off-season. Volta em outubro/2026.\n\n"
                f"Requests restantes: {restantes}"
            )
            return

        texto = f"✅ API OK - {len(dados)} jogos encontrados:\n\n"
        for i, game in enumerate(dados[:3]):
            home = game.get("home_team")
            away = game.get("away_team")
            books = len(game.get("bookmakers", []))
            texto += f"{i+1}. {away} @ {home}\n {books} casas BR\n\n"
...
@app.route('/')
def index():
    return 'Basket Bot v2.8.4 Online'
