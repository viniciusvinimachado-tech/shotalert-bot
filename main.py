...
def buscar_stats_time_espn(team_id, liga_slug):
    ...
    logging.info(f"ESPN Stats Team {team_id}: PPG {resultado['ppg']}")
    return resultado
...
def formatar_alerta_basket(event, nome_liga, antecipado=False):
    try:
        ...
        msg = (
            ...
            f"• Rebotes: {total_rpg} • Ast: {stats['total_apg']} • 3PT: {total_3pm}\n\n"
            + "\n\n".join(alertas)
        )
        return msg
    except Exception as e:
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Basket Bot v2.5 online! ESPN + Props OK")

async def hoje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("🔍 Buscando jogos de hoje...")
    ...
    try:
        ...
        for nome_liga, info in LIGAS.items():
            ...
            for g in games[:8]:
                ...
                txt += f"🕐 {hora_brt.strftime('%H:%M')} — {casa} x {fora}\n"
            msgs.append(txt)

        if not encontrou:
            await msg.edit_text("📭 Nenhum jogo hoje na NBA/WNBA/NCAAB.")
        else:
            await msg.delete()
            for txt in msgs:
        ...
        logging.error(f"Erro /hoje: {e}")
        await msg.edit_text(f"❌ Erro ao buscar jogos")

async def amanha(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("📅 Analisando jogos de amanhã...")
    ...
    try:
        ...
        for nome_liga, info in LIGAS.items():
            ...
            for g in games[:2]:
                alert = formatar_alerta_basket(g, nome_liga)
                if alert:
                    encontrou = True
        ...
        if not encontrou:
            await msg.edit_text("😴 Nenhuma entrada quente amanhã.\nCritério: 215+ pts totais.")
        else:
            await msg.delete()

    except Exception as e:
        logging.error(f"Erro /amanha: {e}")
        await msg.edit_text(f"❌ Erro na análise")

async def antecipados(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("⏰ Buscando odds desreguladas próximos 3 dias...")
    ...
    try:
        ...
        for dias in range(1, 4):
            ...
            for nome_liga, info in LIGAS.items():
                ...
                # Limita 1 jogo por liga/dia pra não travar
                for g in games[:1]:
                    jogos_verificados += 1
                    try:
                        alert = formatar_alerta_basket(g, nome_liga, antecipado=True)
                        if alert:
                            encontrou = True
                            total_alertas += 1
                            await update.message.reply_text(alert, parse_mode="Markdown")
                        ...
                        logging.error(f"Erro jogo {g.get('id')}: {e}")
                        continue

        if not encontrou:
            await msg.edit_text(f"😴 Nenhuma odd desregulada nos próximos 3 dias.\n\n{jogos_verificados} jogos verificados.\nCritério: 215+ pts ou jogador 22+ pts.")
        else:
            await msg.edit_text(f"✅ {total_alertas} oportunidades encontradas em {jogos_verificados} jogos.\n\n💡 Casas abrem linha 2-3 dias antes.")

    except Exception as e:
        logging.error(f"Erro /antecipados: {e}")
        await msg.edit_text(f"❌ Erro na busca ESPN")

async def alerta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("🔥 Analisando jogos de hoje...")
    ...
    try:
        ...
        if not quente:
            await msg.edit_text("😴 Nenhuma entrada quente hoje.\nCritério: 215+ pontos ou jogador 22+ pts.")
        else:
            await msg.edit_text(f"✅ {total_enviado} alertas enviados.")
...
@app.route('/')
def index():
    return 'Basket Bot v2.5 ESPN Online'
