...
LIGAS = {
    ...
    "NCAAB": {"slug": "mens-college-basketball", "id": "41", "odds_sport": "basketball_ncaab"},
}

# ─── THEODDSAPI ───────────────────────────────────────────────────────────────
def buscar_odds_theoddsapi(time_casa, time_fora, sport_key):
    """Busca odds na TheOddsAPI - NBA e WNBA"""
    if not ODDS_API_KEY:
    ...
    try:
        ...
        if resp.status_code!= 200:
            logging.warning(f"TheOddsAPI {sport_key} {resp.status_code}: {resp.text[:200]}")
            return None
        ...
        for game in dados:
            ...
            if (time_casa.lower()[:4] in home or home[:4] in time_casa.lower()[:4]) and \
               (time_fora.lower()[:4] in away or away[:4] in time_fora.lower()[:4]):
                odds_casas = []
...
def formatar_alerta_basket_odds(event, nome_liga, antecipado=False):
    try:
        ...
        sport_key = LIGAS[nome_liga]["odds_sport"]

        if total_ppg < 50: # WNBA tem média menor
        ...
        limite_monitor = 215 if nome_liga == "NBA" else 160
        if total_ppg >= limite_monitor:
            return (
                f"👀 *MONITORAR*\n{'━' * 28}\n"
                f"🏀 *{casa} x {fora}* ({nome_liga})\n"
                f"🕐 {hora_brt.strftime('%d/%m %H:%M')} BRT\n\n"
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v2.8.6 online!\nTheOddsAPI: {'✅' if ODDS_API_KEY else '❌'}")
...
async def debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("🔍 Testando TheOddsAPI NBA + WNBA...")

    try:
        texto = "✅ *TheOddsAPI Status:*\n\n"

        # Testa NBA
        url_nba = "https://api.the-odds-api.com/v4/sports/basketball_nba/odds"
        params = {"apiKey": ODDS_API_KEY, "regions": "us,eu", "markets": "totals"}
        resp_nba = requests.get(url_nba, params=params, timeout=10)
        dados_nba = resp_nba.json() if resp_nba.status_code == 200 else []
        texto += f"🏀 NBA: {len(dados_nba)} jogos\n"

        # Testa WNBA
        url_wnba = "https://api.the-odds-api.com/v4/sports/basketball_wnba/odds"
        resp_wnba = requests.get(url_wnba, params=params, timeout=10)
        dados_wnba = resp_wnba.json() if resp_wnba.status_code == 200 else []
        texto += f"🏀 WNBA: {len(dados_wnba)} jogos\n\n"

        restantes = resp_nba.headers.get('x-requests-remaining', 'N/A')
        texto += f"Requests restantes: {restantes}\n"

        if len(dados_nba) == 0 and len(dados_wnba) == 0:
            texto += "\n😴 Off-season NBA. WNBA pode não ter cobertura no plano grátis."
...
async def antecipados(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏰ Buscando value NBA + WNBA...")
...
@app.route('/')
def index():
    return 'Basket Bot v2.8.6 Online'
