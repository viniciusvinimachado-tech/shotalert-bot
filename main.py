...
ODDS_API_KEY = os.environ.get("ODDS_API_KEY") # Chave grátis TheOddsAPI
...
def buscar_odds_theoddsapi(time_casa, time_fora):
    """TheOddsAPI - Casas BR: Betano, Bet365, KTO"""
    if not ODDS_API_KEY:
    ...
    try:
        ...
        if resp.status_code!= 200:
            logging.warning(f"TheOddsAPI status {resp.status_code}")
            return None

        dados = resp.json()
        logging.info(f"TheOddsAPI: {len(dados)} jogos encontrados")

        for game in dados:
            home = game.get("home_team", "").lower()
            away = game.get("away_team", "").lower()

            # Match Lakers/LAL, Thunder/OKC
            if (time_casa.lower()[:4] in home or home[:4] in time_casa.lower()[:4]) and \
               (time_fora.lower()[:4] in away or away[:4] in time_fora.lower()[:4]):
                ...
                if odds_casas:
                    logging.info(f"Odds BR {time_casa} x {time_fora}: {len(odds_casas)} casas")
                    return odds_casas
        return None
    except Exception as e:
        logging.error(f"TheOddsAPI erro: {e}")
        return None

# ─── SCRAPER ODDS.COM.BR - FALLBACK ───────────────────────────────────────────
HEADERS = {
...
def buscar_odds_br(time_casa, time_fora):
    """Tenta scraper primeiro, se falhar usa TheOddsAPI"""
    try:
        url = "https://www.odds.com.br/basquete/eua/nba/"
        resp = requests.get(url, headers=HEADERS, timeout=10)

        if resp.status_code == 403 or resp.status_code!= 200:
            logging.warning(f"Odds.com.br bloqueou - usando TheOddsAPI")
            return buscar_odds_theoddsapi(time_casa, time_fora)
        ...
        jogos = soup.find_all(['div', 'tr', 'li'], class_=re.compile('event|match|game'))

        for jogo in jogos:
            texto_jogo = jogo.get_text().lower().replace(" ", "")

            if casa_norm[:4] in texto_jogo and fora_norm[:4] in texto_jogo:
                matches = re.findall(r'(\d{3}\.?\d*)\s*@\s*(\d+\.?\d+)', jogo.get_text())

                for match in matches:
                    valor_linha = float(match[0])
                    odd_valor = float(match[1])

                    if 180 < valor_linha < 250:
                        texto_lower = jogo.get_text().lower()
                        casa_aposta = "Bet365"
                        if 'betano' in texto_lower: casa_aposta = "Betano"
                        elif 'kto' in texto_lower: casa_aposta = "KTO"
                        elif 'betfair' in texto_lower: casa_aposta = "Betfair"
        ...
        # Se não achou no scraper, usa API
        return buscar_odds_theoddsapi(time_casa, time_fora)

    except Exception as e:
        logging.error(f"Erro scraper, usando API: {e}")
        return buscar_odds_theoddsapi(time_casa, time_fora)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    api_status = "✅ ON" if ODDS_API_KEY else "❌ OFF - Configure ODDS_API_KEY"
    await update.message.reply_text(
        f"🤖 *Basket Alert Bot v2.8 - TheOddsAPI*\n\n"
        f"🏀 NBA • WNBA • NCAAB\n"
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v2.8 online!\nTheOddsAPI: {'✅' if ODDS_API_KEY else '❌'}")
...
@app.route('/')
def index():
    return 'Basket Bot v2.8 TheOddsAPI Online'
