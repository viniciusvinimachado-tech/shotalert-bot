...
def buscar_odds_theoddsapi(time_casa, time_fora, sport_key):
    ...
    try:
        ...
        def normalizar(nome):
            nome = nome.lower()
            # Remove cidades/estados pra pegar só apelido
            for cidade in ["los angeles", "golden state", "new york", "san antonio", "oklahoma city",
                          "new orleans", "portland", "indiana"]:
                nome = nome.replace(cidade, "")
            return nome.strip().split()[-1] # "lakers", "warriors"
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v2.8.7 online!\nTheOddsAPI: {'✅' if ODDS_API_KEY else '❌'}")
...
@app.route('/')
def index():
    return 'Basket Bot v2.8.7 Online'
