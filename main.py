...
SPORTMONKS_KEY = os.environ.get("SPORTMONKS_KEY") # Sportmonks
...
def buscar_time_sm_id(nome_time):
    ...
    nome_busca = nome_time.replace(" FC", "").replace(" CF", "").replace(" EC", "").replace(" FBPA", "")
    data = sm_request(f"teams/search/{nome_busca}")

    if data and data.get("data"):
        team_id = data["data"][0]["id"]
        team_name = data["data"][0]["name"]
        logging.info(f"Sportmonks MATCH: {nome_time} -> {team_name} [{team_id}]")
        cache_set(key, team_id)
        return team_id
    logging.warning(f"SM: ID não achado {nome_time}")
    return None
...
def buscar_media_stats_sm(nome_time):
    ...
    # Endpoint correto v3: pega últimos 5 jogos com stats
    ...
    for fix in fixtures:
        ...
        if shots > 0 or corners > 0: # Só conta se tiver stats
            total_shots += shots
            total_sot += sot
...
def formatar_alerta_desregulado(match, nome_liga):
    ...
    # FILTROS AJUSTADOS - MAIS FROUXOS
    alertas = []

    # Over Chutes: 18+ já é quente
    if media_shots >= 18:
...
def start(update, context):
    update.message.reply_text(
        "🤖 *Shot Alert Bot v6.8.1 - Sportmonks*\n\n"
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.8.1 online! Sportmonks ativo")
...
@app.route('/')
def index():
    return 'Bot v6.8.1 Sportmonks Online'
