...
# ─── CONFIG DE ESPORTES v3.5 - LIGAS ATIVAS JUN/JUL ───────────────────────────
ESPORTES = {
    "futebol": {
        "nome": "Futebol",
        "emoji": "⚽",
        ...
        "ligas": {
            ...
            "Brasileirão B": {"slug": "bra.2", "id": "100", "odds_sport": "soccer_brazil_serie_b"},
            "MLS": {"slug": "usa.1", "id": "21", "odds_sport": "soccer_usa_mls"},
            "Norueguês": {"slug": "nor.1", "id": "39", "odds_sport": "soccer_norway_eliteserien"},
            "Sueco": {"slug": "swe.1", "id": "40", "odds_sport": "soccer_sweden_allsvenskan"},
            "Finlandês": {"slug": "fin.1", "id": "41", "odds_sport": "soccer_finland_veikkausliiga"},
            "J-League": {"slug": "jpn.1", "id": "27", "odds_sport": "soccer_japan_j_league"},
            "K-League": {"slug": "kor.1", "id": "29", "odds_sport": "soccer_korea_kleague1"},
        },
        "mercado": "totals",
        "limite_monitor": {
            "Brasileirão": 2.3, "Brasileirão B": 2.1, "MLS": 2.8, "Norueguês": 2.9,
            "Sueco": 2.7, "Finlandês": 2.6, "J-League": 2.5, "K-League": 2.4
        },
        "min_ppg": 0.3
    }
}
...
def buscar_odds_theoddsapi(time_casa, time_fora, sport_key):
    ...
    try:
        ...
        if resp.status_code!= 200:
            logging.error(f"TheOddsAPI status {resp.status_code}")
            return None
        ...
        def normalizar(nome):
            nome = nome.lower()
            for termo in ["fc", "cf", "ec", "clube", "atlético", "atletico", "real", "ac", "sc", "de", "ca", "cd"]:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🤖 *Value Bot v3.5 Junho/Julho*\n\n"
        f"{config['emoji']} Modo: {config['nome']}\n"
        f"📋 Ligas ativas: {len(config['ligas'])}\n"
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    restantes = cache_get("requests_restantes_oddsapi")
    alertas = get_alertas_hoje()
    ligas = ", ".join(ESPORTES[get_modo()]["ligas"].keys())
    await update.message.reply_text(
        f"📊 *Status*\n\n"
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v3.5 online!\nAlertas hoje: {get_alertas_hoje()}/3")
...
@app.route('/')
def index():
    return 'Value Bot v3.5 Junho/Julho Online'
