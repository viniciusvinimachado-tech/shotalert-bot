...
def buscar_odds_theoddsapi(time_casa, time_fora):
    ...
    try:
        ...
        if resp.status_code!= 200:
            logging.warning(f"TheOddsAPI {resp.status_code}: {resp.text[:100]}")
            return None
        ...
        for game in dados:
            ...
            if (time_casa.lower()[:4] in home) and (time_fora.lower()[:4] in away):
                ...
                if odds_casas:
                    logging.info(f"Odds {time_casa} x {time_fora}: {odds_casas}")
                    return odds_casas
        return None
...
def espn_request(endpoint):
    ...
    try:
        ...
        if resp.status_code == 200:
            return resp.json()
        logging.warning(f"ESPN {resp.status_code}")
        return None
    except Exception as e:
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v2.8.1 online!\nTheOddsAPI: {'✅' if ODDS_API_KEY else '❌'}")
...
@app.route('/')
def index():
    return 'Basket Bot v2.8.1 Online'
