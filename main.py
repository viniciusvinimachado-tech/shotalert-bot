...
# Mapeamento manual FD -> Sofascore - ATUALIZADO COM TODOS DO SEU LOG
MAPEAMENTO_TIMES = {
    # Premier League
...
def buscar_time_sofascore(nome_time):
    ...
    try:
        # Usa endpoint /search/teams que é mais estável
        url = f"{SOFASCORE_BASE}/search/teams/{nome_busca}"
        resp = SOFASCORE_SESSION.get(url, timeout=10)

        if resp.status_code!= 200:
            logging.warning(f"Sofascore search status {resp.status_code}: {nome_time}")
            return None
...
def buscar_media_stats_time(nome_time):
    ...
    try:
        ...
        for event in events:
            ...
            if resp_stats.status_code!= 200: continue
...
def start(update, context):
    update.message.reply_text(
        "🤖 *Shot Alert Bot v6.7.2*\n\n"
        "🌍 9 ligas • 📊 Média 5 jogos Sofascore\n\n"
        "/ping — Testar\n/jogos — Hoje\n/antecipados — 3 dias\n/alerta — Jogos quentes\n/diagnostico — API",
        parse_mode="Markdown"
    )

def ping(update, context):
    update.message.reply_text("✅ Bot v6.7.2 online! Fix busca times")
