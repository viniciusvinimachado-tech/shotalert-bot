...
def formatar_alerta_basket_odds_br(event, nome_liga, antecipado=False):
    try:
        ...
        time.sleep(1) # Delay pra não tomar block
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 *Basket Alert Bot v2.7.1 - Odds BR*\n\n"
        "🏀 NBA • WNBA • NCAAB\n"
        "💰 Scraper odds.com.br\n"
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Bot v2.7.1 online!\nESPN: ✅\nOdds.com.br: ✅")
...
@app.route('/')
def index():
    return 'Basket Bot v2.7.1 Odds BR Online'
