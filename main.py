...
LIGAS = {
    "Brasileirao": {"fd_id": "2013", "sm_id": 636, "sm_season": 23614},
    "Premier League": {"fd_id": "2021", "sm_id": 8, "sm_season": 23690},
    "La Liga": {"fd_id": "2014", "sm_id": 564, "sm_season": 23737},
    "Serie A Italia": {"fd_id": "2019", "sm_id": 384, "sm_season": 23715},
    "Bundesliga": {"fd_id": "2002", "sm_id": 82, "sm_season": 23776},
    "Ligue 1": {"fd_id": "2015", "sm_id": 301, "sm_season": 23716},
    "Champions League": {"fd_id": "2001", "sm_id": 2, "sm_season": 23748},
    "Europa League": {"fd_id": "2146", "sm_id": 5, "sm_season": 23749},
    "Libertadores": {"fd_id": "2152", "sm_id": 876, "sm_season": 23882},
}
...
def cache_get(key):
    item = _cache.get(key)
    if item and time.time() - item["time"] < 86400: # 24h pra stats de temporada
        return item["data"]
    return None
...
def buscar_media_stats_sm(nome_time, liga_nome):
    ...
    season_id = LIGAS[liga_nome]["sm_season"]

    if not team_id:
        logging.warning(f"SM: ID não mapeado {nome_time}")
        return {"media_shots": 0, "media_sot": 0, "media_corners": 0, "media_goals": 0, "jogos": 0}

    # Endpoint de stats de temporada - funciona no grátis
    data = sm_request(f"teams/{team_id}/statistics?season_id={season_id}")
    ...
    # IDs dos types de stats no Sportmonks
    shots = stats.get("shots", {}).get("total", 0)
    sot = stats.get("shots_on_target", {}).get("total", 0)
    corners = stats.get("corners", {}).get("total", 0)
    goals = stats.get("goals", {}).get("for", 0)
...
def start(update, context):
    update.message.reply_text(
        "🤖 *Shot Alert Bot v6.8.4 - Sportmonks*\n\n"
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.8.4 online! Sportmonks stats temporada OK")
...
@app.route('/')
def index():
    return 'Bot v6.8.4 Sportmonks Online'
