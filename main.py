...
# ─── CONFIG DE ESPORTES v3.4 - LIGAS ATIVAS JUN/JUL ───────────────────────────
ESPORTES = {
    "futebol": {
        "nome": "Futebol",
        "emoji": "⚽",
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    restantes = cache_get("requests_restantes_oddsapi")
    alertas = get_alertas_hoje()
    ligas = ", ".join(ESPORTES["futebol"]["ligas"].keys())
    await update.message.reply_text(
        f"📊 *Status*\n\n"
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ Bot v3.4 online!\nAlertas hoje: {get_alertas_hoje()}/3")
...
@app.route('/')
def index():
    return 'Value Bot v3.4 Junho/Julho Online'
