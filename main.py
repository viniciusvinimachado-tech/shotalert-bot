...
# Mapeamento manual FD -> Sofascore pra times que dão problema
MAPEAMENTO_TIMES = {
...
def buscar_time_sofascore(nome_time):
    ...
    try:
        url = f"{SOFASCORE_BASE}/search/all/{nome_busca}"
...
def buscar_media_stats_time(nome_time):
    ...
    if not team_id:
        logging.warning(f"Team ID não achado: {nome_time}")
...
def ping(update, context):
    update.message.reply_text("✅ Bot v6.7.1 online! Mapeamento ativo")
